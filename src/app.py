...
from client import ImageProviderClient
import io


IMGS_IDX = set([10022, 9965])
IMGS_HOST = 'http://158.160.146.180:7777'
...
def read_plate_number():
    ...
    for image_id in image_ids:
        try:
            ...
            return {'error': f'image_id must be integer: {image_id}'}, 400

        if image_id not in IMGS_IDX:
            return {'error': f'image_id not found: {image_id}'}, 404
        ...
        try:
            ...
            return {'error': 'error accessing the server'}, 500

        try:
            image = io.BytesIO(image)
        except TypeError:
            return {'error': 'the server should return jpeg in '
                    'binary form'}, 500
